document.addEventListener('DOMContentLoaded', function() {
	...
	document.querySelectorAll('.edit-btn').forEach(button => {
		button.addEventListener('click', function(event) {
			...
			var newActionUrl = '/auth/gymOwner/updateClass';
    ...
    var classInstructorId = button.data('class-instructor');
    ...
    var classDuration = button.data('class-duration');
    var classDaysOfWeek = button.data('class-days-of-week');

    // Convertir los días de la semana a un array de strings
    var daysOfWeekArray = classDaysOfWeek.split(',');

    var modal = $(this);

    modal.find('.modal-body #editClassId').val(classId);
    ...
    modal.find('.modal-body #editClassDuration').val(classDuration);

    // Establecer los días de la semana en el campo select múltiple
    var selectDaysOfWeek = modal.find('.modal-body #editClassDaysOfWeek');
    selectDaysOfWeek.val(daysOfWeekArray);  // Esto no funciona directamente para seleccionar opciones múltiples

    // Necesitamos seleccionar cada opción individualmente
    daysOfWeekArray.forEach(function(day) {
        selectDaysOfWeek.find('option[value="' + day + '"]').prop('selected', true);
    });

    // Lógica AJAX para poblar select de instructores omitida por simplicidad en este ejemplo
});
